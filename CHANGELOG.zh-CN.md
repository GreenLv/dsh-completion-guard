...
- 增加共享 core/v2 消费端，并以源码镜像 pin 绑定 Codex Context Guard 提交 `7c97ea0d2790d2a3c14ef80d0ddb09a16ab8bf9a`，精确列出 fixture 与运行时 JSON 摘要；历史 v1 pin 保持不变。v6 会话边界把新运行时与旧 v5 历史记录分开。
- 退役普通 Guard action/evidence 执行，改用只读文件、Git 与包脚本就绪观察。评估可以选择稳定的既有输入，不强迫再编辑；根要求中的时间或审批条件仍待独立满足。原生观察使用独立版本化摘要域；旧 v3 编码和证书保持历史字节。
- 相对文件及目录约束保留根时 Session 定位来源，并经文件系统规范路径回读核验。当前 v6 证书以第 4 版绑定该定位身份；测试或测量的非路径目标保持独立身份。
- 宿主确实修改被禁止的文件时，即使获准的编辑也成功，该有源违例仍阻断完成；最终回答中的自述不视为宿主变更。
- 增加合成 Session 回放 CLI，用于源码到 core 以及 Stop/Goal 诊断；它不代表真实模型或原生平台验收。
- 宿主结果中的结构化错误优先于看似成功的渲染文本；畸形或归属不明的结果保持未知。0.7 原生 profile 检查真实宿主注册与普通证据，保留的显式 release 门仅使用本机 mock registry 验证。
- 将前置工作区地点及并列的编辑、测试、回读、最终回答要求分别保留为当前义务；方法附句不能悄然关闭独立工作。创建仍需可信的写入前不存在证据；已审计的测试／基准观察器不认证任意包脚本的测量数值。
- 同一轮可信最终回答现在可以交付多个彼此独立的信息要求。每项要求拥有独立的交付事实，同时保留共同的回答来源；较早的回答仍不能交付后来的要求。
- 有根来源的持续、暂停、取消与后续恢复只绑定指令持久化时已经存在的要求。原根直接并列的编辑及同对象测试可组成有来源的修复闭包；无关测试与显式 proof 仍保持独立。复合继续指令不会再产生第二条 generic 业务待办。旧要求的失效在较早水位回放时不会被倒填。
- 明确要求使用只读观察器的方法附句保留为独立、有来源的义务；只有同一当前文件或测试要求的宿主调用与已持久化观察结果相符，才可关闭它。旧 generic 子句不会因重放而升级。具名、前台的 `npm test` 或 `pnpm test` 在核实已安装宿主渲染器的精确字节及终态后，可无需未被要求的 Guard 就绪调用而完成普通“运行测试”义务。完整结果没有终止标记时可证明退出码为零；形似非零退出标记的文本不能证明具体退出码或准确的失败报告。“运行并报告”和“使测试通过”仍是不同要求。
- 默认 v6 的 prepare 与 checkpoint 反馈现在读取已确认水位的当前 core 闭包：已经观察满足的普通测试或编辑不会再被列为旧绑定待办。未交付的报告仍待实际最终回答；缺失 core 证据保持未知。显式 proof、Goal 采用和适用的 release 合同继续执行各自的证书检查。

### 验证

此前候选字节通过了共享 core fixture 和完整本地确定性矩阵，包括仅在一次性隔离测试夹具内进行的 Git 提交与推送。结构化宿主结果修复及新版原生驱动需要重新执行源码、制品和原生检查；精确结果记录在仓库外的协调交接中。独立的合成 Session 与真实本地文件系统观察器回放覆盖 Stop 和 Goal 的正反路径；它不等于真实模型或原生平台验收。正式发布打包器要求产品 Git 工作树干净。日常安装与发布仍未验收。
