...
- core/v2 消费端按 Codex Context Guard 提交 `7c97ea0d2790d2a3c14ef80d0ddb09a16ab8bf9a` 精确镜像共享源码与一致性夹具。旧 v1 pin 和 v5 历史保持原状；源码身份不等于两个产品完全等价。
...
候选 `583035bd90b8ee589d01b12487a057b655d43e41` 通过完整本地确定性矩阵、候选 CI，以及同一干净源码 tgz 的 macOS 原生 34 项门禁和清理。该原生运行跳过真实模型请求；Windows 同一制品和完整模型验收仍待完成。若打包文档或源码改变，这些结果只属于原字节。最终发布候选、日常安装、npm 发布、tag 和 GitHub Release 均尚未验收或读回；详见[验收记录](docs/LOCAL_ACCEPTANCE.md)。
