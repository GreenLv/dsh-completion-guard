...
const BOUNDARY_RECORD_PREFIX = "[context-guard boundary/v1] ";
...
function currentActionBases(projection, enforceCore = true) {
	...
	for (const item of projection.items.values()) {
		...
		const latestRun = [...projection.evidence.values()].filter((fact) => fact.epoch === projection.epoch && fact.toolResultSeq >= sourceSeq$1 && fact.semanticAction === action && fact.evidenceRole === "effect" && fact.subjects.includes(scope) && fact.parseStatus === "supported" && fact.processFacts?.operationAttribution === "single_operation").sort((a, b) => b.toolResultSeq - a.toolResultSeq)[0];
...
function normalizeReleaseContract(raw, adoptedBy, adoptedAtRevision = 0) {
	...
	const candidate = asRecord$2(value.candidate);
...
function normalizeSettlement(raw) {
	...
	const readbackRaw = asRecord$2(value.readback);
...
*/
function verifiedCore(projection) {
	const core = projection.coreV2;
	if (projection.integrity !== "valid" || projection.durabilityWatermark !== "confirmed" || core?.schema !== "core-state/v2" || typeof core.certifiable !== "boolean" || !core.predicates || typeof core.predicates !== "object" || Array.isArray(core.predicates) || !Array.isArray(core.unmet_requirements) || !Array.isArray(core.current_actions)) return { reason: "core_projection_unavailable" };
	const predicates = core.predicates;
	const openIds = core.unmet_requirements;
	if (openIds.some((id) => typeof id !== "string" || !Object.hasOwn(predicates, id)) || Object.values(predicates).some((value) => typeof value !== "string")) return { reason: "core_projection_unavailable" };
	if ([...openIds, ...Object.entries(predicates).filter(([, state]) => state.startsWith("constraint_")).map(([id]) => id)].some((id) => !sourceItemForCoreRequirement(projection, id))) return { reason: "core_requirement_source_unavailable" };
	return { core: {
...
* release.
*/
function v6VerifiedCoreConditions(projection) {
	if ("reason" in verifiedCore(projection)) return void 0;
	const conditions = projection.coreV2?.conditions;
	if (!conditions || typeof conditions !== "object" || Array.isArray(conditions)) return void 0;
	return Object.fromEntries(Object.entries(conditions).filter((entry) => typeof entry[1] === "string"));
}
/**
...
* R4F1/R5).
*/
function isV6PendingRootWait(projection, item) {
	if (!item || item.status !== "pending") return false;
	if (item.waitAuthorization === void 0 && item.authorityDisposition !== "conditional_wait") return false;
	const current = projection.currentUnitId;
	if (item.unitId === void 0 || current === void 0) return false;
...
function currentV6Feedback(projection) {
	...
	if (!verified.core.certifiable && verified.core.openIds.length === 0) return {
		status: "unknown",
...
* ordinary execution.
*/
const V6_ORDINARY_COMPLETION_RULE = "Ordinary completion is read from persisted host tool results and independent readback; uncertified work is delivered honestly and stays uncertified. No Guard checkpoint, binding, or target re-authorization is a prerequisite for ordinary work.";
/** The same rule at a medium budget. */
const V6_ORDINARY_COMPLETION_RULE_SHORT = "Ordinary completion is read from persisted host results; uncertified work is delivered honestly and needs no Guard checkpoint or re-authorization.";
/** The same rule at emergency budget — still never a checkpoint prerequisite. */
const V6_ORDINARY_COMPLETION_RULE_COMPACT = "Ordinary completion is read from persisted host results; no Guard checkpoint or re-authorization is required.";
function v6CompletionRuleFor(budget) {
	if (budget >= DEFAULT_RECOVERY_CHAR_BUDGET) return V6_ORDINARY_COMPLETION_RULE;
	if (budget >= 1e3) return V6_ORDINARY_COMPLETION_RULE_SHORT;
...
function currentLineagePendingItems(projection) {
	const current = projection.currentUnitId;
	if (current === void 0) return [];
	const lineage = new Set([current, ...unitAncestorIds(projection, current)]);
	return [...projection.items.values()].filter((item) => item.status === "pending" && item.unitId !== void 0 && lineage.has(item.unitId));
}
/**
...
*/
function v6CurrentRootBoundaries(projection) {
	const sort = (a, b) => b.revision - a.revision || a.id.localeCompare(b.id);
...
*/
function renderV6RecoveryPacket(projection, current, budget) {
	...
	const isPendingWait = (item) => isV6PendingRootWait(projection, item);
	const { prohibitions: durableProhibitions } = v6CurrentRootBoundaries(projection);
	const cleanupApplies = currentLineagePendingItems(projection).some((item) => carriesCleanupCondition(deriveItemDiagnosis(projection, item).capability.gap));
	const waitBoundaryRow = (item) => `[${clip(item.id, 20)}] root_condition_pending; wait for trusted root: ${item.resumeEvent ?? item.condition ?? item.normalizedText}; do not execute before release`;
	const waitBoundaryReference = (item) => `[${clip(item.id, 20)}] root_condition_pending; exact condition: context_guard_prepare; do not execute before release`;
	const constraintBoundaryRow = (item, state) => {
	...
	const constraintBoundaryReference = (item) => `DO NOT [${clip(item.id, 20)}] exact text: context_guard_prepare`;
	if (current.status === "unknown") {
		...
		if (projection.integrity !== "valid") lines.push(compact ? "Root source integrity is unverifiable: known constraints cannot be safely recovered here; do not treat any boundary as preserved." : "Root source integrity is unverifiable, so the known prohibitions and waits cannot be safely recovered in this packet; do not treat any boundary as preserved, and re-establish them with the root before acting.");
		else {
			const { waits } = v6CurrentRootBoundaries(projection);
			const footer$1 = (constraintsShown$2, waitsShown$2) => compact ? `${durableProhibitions.length - constraintsShown$2} constraint folded; ${waits.length - waitsShown$2} wait folded.` : `${durableProhibitions.length - constraintsShown$2} constraint rows folded; ${waits.length - waitsShown$2} wait rows folded.`;
			let remaining$1 = budget - (lines.join("\n").length + 82 + footer$1(0, 0).length + 3);
			const budget2 = makeV6RowBudget(lines, unknownRuleTiers, ruleLineIndex, () => remaining$1, (value) => {
				remaining$1 = value;
			});
			let constraintsShown$1 = 0, waitsShown$1 = 0;
			for (const item of durableProhibitions.slice(0, compact ? 1 : 4)) if (budget2.addBoundary(constraintBoundaryRow(item), constraintBoundaryReference(item))) constraintsShown$1++;
			for (const item of waits.slice(0, compact ? 1 : 4)) if (budget2.addBoundary(waitBoundaryRow(item), waitBoundaryReference(item))) waitsShown$1++;
			lines.push(footer$1(constraintsShown$1, waitsShown$1));
		}
		lines.push(pointer);
		return lines.join("\n");
	}
	...
	if (current.reasonCode === "legacy_record_needs_review") {
		const shown$1 = current.openIds.slice(0, 2).map((id) => clip(id, 20)).join("; ");
	...
	const requirementRow = (id) => {
		const state = current.predicates[id];
		const sourced = sourceItemForCoreRequirement(projection, id);
		const item = sourced?.item;
		if (state === "insufficient" && sourced?.origin !== void 0) return {
			reason: state,
			body: compact ? `run the read-only ${sourced.origin.action}; its persisted result records it` : `run the read-only ${sourced.origin.action} named by the root requirement; its persisted result records this method`
		};
		if (state === "insufficient" && item !== void 0 && (item.taskKind === "inquiry" || item.authorityDisposition === "informational")) return {
			reason: state,
			body: compact ? "deliver the answer; the completed turn closes it" : "deliver the actual answer; the final response of the completed turn closes it"
		};
		if (state === "insufficient") return {
			reason: state,
			body: compact ? "unmet from persisted facts; do it with host tools; never re-run a completed action" : "still unmet from persisted host facts; do the work with host tools and let the persisted result or an independent readback show it; never re-run a completed action to mint evidence"
		};
		if (state === "legacy_review") return {
			reason: state,
			body: compact ? "not certifiable from current facts; ordinary host work continues; stays uncertified" : "recorded but not certifiable from current confirmed facts; continue ordinary host work; stays uncertified unless a fresh root instruction adopts a certified path"
		};
		if (state === "constraint_violated") return {
			reason: state,
			body: compact ? "a sourced host mutation violated this prohibition; report; do not repeat" : "a sourced host mutation violated this prohibition; report it; do not repeat it"
		};
		if (state === "constraint_unresolved") return {
			reason: state,
			body: compact ? "host facts cannot establish this prohibition was respected" : "current host facts cannot establish whether this prohibition was respected"
		};
		return {
			reason: state ?? "unmet",
			body: "answer this sourced requirement with the current Host observation or final delivery"
		};
	};
	const openRows = current.status === "incomplete" ? current.openIds : [];
	const waitItems = openRows.map((id) => sourceItemForCoreRequirement(projection, id)?.item).filter((item) => isPendingWait(item));
	...
	const footer = (constraintsShown$1, waitsShown$1, shown$1) => compact ? `${durableProhibitions.length - constraintsShown$1} constraint folded; ${waitItems.length - waitsShown$1} wait folded; ${workRows.length - shown$1} work folded.` : `${durableProhibitions.length - constraintsShown$1} constraint rows folded; ${waitItems.length - waitsShown$1} wait rows folded; ${workRows.length - shown$1} requirement rows folded. Current scope is the confirmed core; older records stay historical.`;
	let remaining = budget - (lines.join("\n").length + 82 + footer(0, 0, 0).length + 3);
	const rowBudget = makeV6RowBudget(lines, [
		v6CompletionRuleFor(budget),
		budget >= DEFAULT_RECOVERY_CHAR_BUDGET ? V6_ORDINARY_COMPLETION_RULE_SHORT : V6_ORDINARY_COMPLETION_RULE_COMPACT,
		V6_ORDINARY_COMPLETION_RULE_COMPACT
	].filter((tier, index$1, tiers) => tiers.indexOf(tier) === index$1), ruleLineIndex, () => remaining, (value) => {
		remaining = value;
	});
	let constraintsShown = 0, waitsShown = 0, shown = 0;
	const constraintLimit = compact ? 1 : 4;
	for (const item of durableProhibitions.slice(0, constraintLimit)) if (rowBudget.addBoundary(constraintBoundaryRow(item, current.predicates[item.id]), constraintBoundaryReference(item))) constraintsShown++;
	const waitLimit = compact ? 1 : 4;
	for (const item of waitItems.slice(0, waitLimit)) if (rowBudget.addBoundary(waitBoundaryRow(item), waitBoundaryReference(item))) waitsShown++;
	const rowLimit = compact ? 1 : 4;
	for (const id of workRows.slice(0, rowLimit)) {
		const row$3 = requirementRow(id);
		const item = sourceItemForCoreRequirement(projection, id)?.item;
		const text = item ? clip(item.normalizedText, 70) : "";
		if (rowBudget.add(`[${clip(id, 20)}] ${row$3.reason}; ${row$3.body}; ${text}`, compact ? 110 : 310)) shown++;
	}
...
*/
function makeV6RowBudget(lines, ruleTiers, ruleLineIndex, getRemaining, setRemaining) {
	const tryWhole = (line) => {
	...
	const shortenRule = () => {
		const currentRule = lines[ruleLineIndex];
	...
	const addBoundary = (full, reference) => {
		if (tryWhole(full)) return true;
		while (shortenRule()) if (tryWhole(full)) return true;
...
function renderLegacyRecoveryPacket(projection, options, budget) {
	const clip = (text, size) => text.length <= size ? text : text.slice(0, size - 1) + "…";
...
function persistedToolResultStatus(data, callId, kind = "tool/result", matchedStart = false) {
	...
	const message = asRecord$1(result.message);
	const source = asRecord$1(message?.source);
	if (message?.isError === true) return "failure";
	if (message?.role !== "tool" || source?.kind !== "tool" || source.callId !== callId || message.toolCallId !== callId || message.isError !== false || !Array.isArray(message.content) || message.content.some((block$1) => asRecord$1(block$1)?.type === "tool-result")) return "unknown";
	return "clean";
}
...
function declaredOperationResults(meta) {
	const declared = asRecord$1(asRecord$1(meta)?.contextGuardProcess)?.operationResults;
	if (!Array.isArray(declared) || declared.length === 0 || declared.length > 64) return void 0;
	const rows = [];
	for (const raw of declared) {
		const row$3 = asRecord$1(raw);
...
function declaredStructuredTerminal(meta) {
	const record = asRecord$1(asRecord$1(meta)?.contextGuardProcess);
	if (!record) return void 0;
	const rawExit = record.exitCode ?? record.exit_code;
...
function extractToolSubject(call, result, defaultCwd, hostLock) {
	...
	if (call.name === "context_guard_observe_file") {
		const native = asRecord$1(asRecord$1(result.meta)?.contextGuardNativeFile);
	...
	if (call.name === "context_guard_observe_git") {
		const native = asRecord$1(asRecord$1(result.meta)?.contextGuardNativeGit);
	...
	if (call.name === "context_guard_observe_test_readiness") {
		const ready = asRecord$1(asRecord$1(result.meta)?.contextGuardTestReadiness);
...
function observerMethodEvidence(projection, events, method, index$1) {
	...
	return [...projection.evidence.values()].find((fact) => {
		...
		const results = events.filter((event) => {
			...
			const message = row$2(row$2(event.data).message);
			return message.role === "tool" && message.toolCallId === fact.callId && row$2(message.source).callId === fact.callId;
		});
		...
		switch (event.type) {
			...
			case "user/message": {
				...
				if (isProtocolBoundaryNotice(event) || isProtocolBoundaryNotice(event, CAPTURE_V042_NOTICE) || isProtocolBoundaryNotice(event, PROTOCOL_V4_NOTICE)) break;
				{
					...
					if (enabled && recordSource?.kind === "context-guard" && recordSource.plugin === "context-guard" && recordSource.form === "notice" && recordText.startsWith(BOUNDARY_RECORD_PREFIX)) {
						...
						const candidate = request && qualifyBoundary(projection, {
							...
							qualificationKind: String(request.qualificationKind),
						...
						} else if (!projection.boundaries.some((entry) => entry.candidateSha256 === candidate.candidateSha256)) projection.boundaries.push(candidate);
						break;
					}
...
function snapshotSessionEvents(session) {
	...
	if (typeof source.snapshotEvents !== "function") throw new SessionApiError("session does not expose the DSH Session V4 snapshotEvents() API");
...
function hostWorkdirForCall(events, call, result, rootSeq, sessionDigest, hostDigest, headerCwd) {
	...
	const upperBound = events.find((event) => event.seq > result.seq && (event.type === "assistant/message" || event.type === "turn/end"))?.seq ?? Number.POSITIVE_INFINITY;
	const candidates = events.filter((event) => event.seq > result.seq && event.seq < upperBound && event.type === "user/message" && ["plugin", "context-guard"].includes(String(row$1(row$1(event.data).source).kind)) && row$1(row$1(event.data).source).plugin === "context-guard" && row$1(row$1(event.data).source).form === "notice" && Array.isArray(row$1(event.data).content) && String(row$1(row$1(event.data).content[0]).text ?? "").startsWith(HOST_WORKDIR_PREFIX)).filter((event) => {
		try {
...
function sessionCoreSnapshot(events, projection, displayOrigins) {
	...
	for (const item of currentItems) {
		...
		const isDirectTestFact = (fact) => {
			...
			const pair = sourceByCall.get(fact.callId);
			if (!pair || pair.call.seq <= root.seq || pair.result.seq !== fact.toolResultSeq || persistedToolResultStatus(pair.result.data, fact.callId) === "unknown") return false;
			let args = {};
			try {
		...
		if (kind === "information" && item.answeredBy) {
			...
			if (delivery && item.answeredBy.turn === Number(turn)) {
				...
				const runPair = run ? sourceByCall.get(run.callId) : void 0;
				const persistedRunStatus = runPair ? persistedToolResultStatus(runPair.result.data, run.callId) : "unknown";
				const declaredExit = terminalZero ? 0 : persistedRunStatus === "failure" && typeof run?.processFacts?.declaredExitCode === "number" ? run.processFacts.declaredExitCode : void 0;
				const assertedText = deliveryAssertionText(deliveredText);
				const reportedExits = [...assertedText.matchAll(/(?:exit\s*(?:code|status)?|退出码)\s*[:=]?\s*(\d+)(?!\d)(?!\.\d)/giu)].map((match) => Number(match[1]));
				const trustedFailure = persistedRunStatus === "failure" && run?.processFacts?.outcome === "failure";
				const conflictingStatus = (trustedFailure ? /(?:test(?:s)?\s+(?:passed|succeeded)|测试(?:已)?通过|测试成功)/iu.test(assertedText) : /(?:test(?:s)?\s+(?:failed|did\s+not\s+pass)|测试(?:未通过|失败))/iu.test(assertedText)) || reportedExits.length > 0 && (declaredExit === void 0 || reportedExits.some((value) => value !== declaredExit));
...
//#endregion
export { PROOF_KINDS_V2 as $, CERTIFICATE_VERSION_V2 as $i, partialFailureOf as $n, COMMAND_SURFACE_MANIFEST as $r, bindingSatisfies as $t, PROTOCOL_V4_NOTICE as A, GOAL_HOST_PACKAGES as Ai, qualifyBoundary as An, isExecutableItem as Ar, cleanupConditionFor as At, extractToolSubject as B, selectHostCohort as Bi, CONFIRM_LINE_PATTERN as Bn, maskQuotedSpans as Br, OUTCOME_STRENGTH as Bt, SESSION_EVENT_ENVELOPE_INVALID as C, verifyComposedHostLockDump as Ci, certifiableOpenItems as Cn, governedClauseRestrictsExecution as Cr, CLEANUP_CONDITION_RULE_SHORT as Ct, CAPTURE_V042_NOTICE as D, BASE_HOST_PACKAGES as Di, availableBoundaryQualifications as Dn, interpretClause as Dr, V6_ORDINARY_COMPLETION_RULE_COMPACT as Dt, projectCoreV2 as E, ACTIVE_HOST_LAUNCHER_VERSION as Ei, BOUNDARY_RECORD_PREFIX as En, hasWorkPredicate as Er, V6_ORDINARY_COMPLETION_RULE as Et, legacyRecordsNeedingReview as F, evaluateExternalWaitCapability as Fi, proposeRebindOutcome as Fn, isRestatement as Fr, renderRecoveryPacket as Ft, isRunExecutable as G, compareHostVersions as Gi, evidenceAvailabilityReason as Gn, qualificationOfClause as Gr, normalizeReleaseContract as Gt, persistedToolResultStatus as H, MIN_SUPPORTED_HOST_VERSION as Hi, parseConfirmationMessage as Hn, opensConditionLead as Hr, RELEASE_OPERATION_SURFACES as Ht, rootLocatorFlavor as I, evaluateHostCapability as Ii, proposeRebindV042 as In, itemHoldsExecutionAuthority as Ir, v6CurrentRootBoundaries as It, authorityCaptureCounts as J, satisfiesSupportedHostRange as Ji, DEPENDENCY_FREE_ONLY_CONDITION as Jn, restatedContentOf as Jr, readbackSettlesContract as Jt, parsePwshCommand as K, evaluateMinimumHostVersion as Ki, itemDiagnosis as Kn, questionHeadsClause as Kr, normalizeReservation as Kt, supersedeItem as L, evaluateHostLock as Li, rebindAttemptKey as Ln, kindOfScope as Lr, currentV6Feedback as Lt, PROTOCOL_V6_NOTICE as M, HOST_COHORTS as Mi, createProjection as Mn, isInformationalFragment as Mr, openItems as Mt, applyUpgradeEligibility as N, bindExecutableIdentity as Ni, confirmRebind as Nn, isOpenObligation as Nr, recoveryDigest as Nt, DEFAULT_DELEGATION_TOOL_NAMES as O, DEFAULT_HOST_LOCK as Oi, effectuateBoundary as On, interpretMessage as Or, V6_ORDINARY_COMPLETION_RULE_SHORT as Ot, deriveProjection as P, bindLiveGoalCapability as Pi, proposeRebind as Pn, isQuestionScopeNeedingReview as Pr, recoveryTitle as Pt, PROOF_KINDS as Q, CERTIFICATE_VERSION as Qi, capabilityFactOf as Qn, verbIsNegated as Qr, reservationFor as Qt, evidenceFromPersistedToolResult as R, evaluateToolSurfaceCapability as Ri, rebindResponse as Rn, legacyQuestionReadingIsInformational as Rr, isV6PendingRootWait as Rt, SESSION_API_UNSUPPORTED as S, resolveInstalledHostLock as Si, hasCurrentCertificate as Sn, explanationHasActionResidue as Sr, CLEANUP_CONDITION_RULE_COMPACT as St, snapshotSessionEvents as T, ACTIVE_HOST_COHORT_IDS as Ti, unitDescendantIds as Tn, hasQuestionScope as Tr, MIN_RECOVERY_CHAR_BUDGET as Tt, withDurability as U, SUPPORTED_HOST_RANGE as Ui, capabilityRemedyPhrase as Un, opensWithDirective as Ur, contractById as Ut, isDeterministicCheck as V, LATEST_SUPPORTED_HOST_VERSION as Vi, isFrozenV042RebindResponse as Vn, namedActions as Vr, RELEASE_OPERATIONS as Vt, canonicalArgvFromCommand as W, SUPPORTED_HOST_VERSIONS as Wi, deriveItemDiagnosis as Wn, presentExplanationHead as Wr, inFlightReservation as Wt, certifyCheckpoint as X, ACTION_MANIFEST_VERSION as Xi, admissibleForRemoval as Xn, splitTextFragments as Xr, releaseCoverage as Xt, segmentAuthorityBlocks as Y, ACTION_MANIFEST as Yi, actionHasCertificationPath as Yn, semanticActionOfScope as Yr, releaseContractFor as Yt, PROOF_CAPABILITY_MATRIX as Z, BOUNDED_ARTIFACT_TYPES as Zi, capabilityConsequence as Zn, statefulActionsOfScope as Zr, releasePreEffectDecision as Zt, projectSessionCoreV2 as _, inspectTargetHostGraph as _i, observeAssistantOutcome as _n, actionVerbMatches as _r, sessionQuery as _t, createGitPrestateEnvelope as a, actionCompatible as aa, sanitizeUrl as ai, NO_PROGRESS_TURNS_BEFORE_STOP as an, environmentDefaultRepositoryTarget as ar, canonicalProjection as at, captureHostWorkdir as b, readActiveHostGraph as bi, v6TestPredicate as bn, clauseIsGoverned as br, validateProofManifestV2 as bt, parseGitCommandManifest as c, requestedIdentityKey as ca, activeRendererModule as ci, classifyCompletionClaim as cn, extractOperation as cr, proofCapabilityReport as ct, FIRST_STEP_GUIDANCE as d, semanticActionFromCommand as da, auditedForegroundRenderers as di, decideTurnStopping as dn, canonicalRegistryBase as dr, proofEvidenceConstraints as dt, SEMANTIC_ACTIONS as ea, validateManifest as ei, evidenceCoverage as en, removalIsComplete as er, PROOF_MANIFEST_DOMAIN_V2 as et, claimedBatchHasRealRootInput as f, semanticActionFromText as fa, auditedHostImplementation as fi, decisionBoundaryKey as fn, npmEscapedPackageName as fr, proofHostSurfacesOf as ft, previewFirstStepInjection as g, injectActiveProfileHostLock as gi, latestRootInstruction as gn, LEGACY_QUALIFICATION as gr, scopeCoverageDigest as gt, lifecyclePhase as h, RC017_RC2_HOST_PACKAGES as ha, hostLockRowsFromComposedDump as hi, latestAssistantText as hn, GRANTED_QUALIFICATION as hr, requiredSubjectsOf as ht, commitTreeSnapshotDigest as i, SUPPORTED_EVIDENCE_ADAPTERS as ia, sanitizeClauseText as ii, NO_PROGRESS_RECORD_PREFIX as in, classifyClause as ir, bindProofV2ToProjection as it, PROTOCOL_V5_NOTICE as j, HOST_CAPABILITY_PACKAGE_GROUPS as ji, currentContractDigest as jn, isExplanationScope as jr, closingHint as jt, PROTOCOL_V3_NOTICE as k, EXPECTED_HOST_PACKAGES as ki, isCurrentAcceptedBoundary as kn, introducesActionClause as kr, carriesCleanupCondition as kt, revalidateGitPrestate as l, requestedTargetAuthorizesMutation as la, auditedDefaultWorkdirHost as li, currentActionBases as ln, isInformationalMessage as lr, proofDigest as lt, firstStepGuidanceV6 as m, validateActionTarget as ma, hostLockContextFromComposedDump as mi, isWholeTaskCompletionClaim as mn, classifyUserInteraction as mr, proofV2Rejection as mt, GIT_COMMAND_TEMPLATES as n, STOP_PROTOCOL_VERSION as na, digestStrings as ni, isVerifyingCapability as nn, captureClause as nr, PROOF_PROTOCOL_VERSION_V2 as nt, executeRevalidatedGitEffect as o, boundedArtifactChoiceMatches as oa, sha256 as oi, assessmentAction as on, extractArtifactPaths as or, createProofManifest as ot, firstStepGuidance as p, validateActionManifest as pa, combineHostPolicy as pi, isRootPauseRequest as pn, classifyTaskIntent as pr, proofOperationMatches as pt, parseShellCommand as q, parseHostVersion as qi, relevantEvidence as qn, reportingHeadGoverns as qr, normalizeSettlement as qt, commitIndexSnapshotDigest as r, STOP_PROTOCOL_VERSION_V2 as ra, normalizeClause as ri, CONTROL_RECORD_PREFIX as rn, captureItem as rr, bindProofToProjection as rt, gitCommandMatchesTarget as s, isStatefulAction as sa, HostProfileError as si, assessmentOutcomePredicate as sn, extractMethod as sr, createProofManifestV2 as st, GIT_COMMAND_MANIFEST_IDS as t, STATEFUL_ACTIONS as ta, canonicalizePath as ti, evidenceMatchesItem as tn, removalIsPartiallyKnown as tr, PROOF_PROTOCOL_VERSION as tt, verifiedLinearCommitReadback as u, requestedTargetMatchesResolved as ua, auditedDefaultWorkdirProvider as ui, decideTurnBoundary as un, segmentClauses as ur, proofDigestV2 as ut, sessionCoreSnapshot as v, packageRowsFromActiveGraph as vi, progressFingerprint as vn, clarifiedSpanOf as vr, sessionQueryV2 as vt, SessionApiError as w, ACTIVE_HOST_COHORT_ID as wi, certificateClosure as wn, hasOrderedCoordination as wr, DEFAULT_RECOVERY_CHAR_BUDGET as wt, sourcedNamedTestRoot as x, resolveActiveProfileHostLock as xi, goalCompletionDenial as xn, clauseIsProtected as xr, CLEANUP_CONDITION_RULE as xt, HOST_WORKDIR_PREFIX as y, packageRowsFromPnpmLock as yi, testOutcomePredicate as yn, clauseAsksOwnQuestion as yr, validateProofManifest as yt, extractTextContent as z, hostVersionFromPackages as zi, replayRebindResult as zn, maskCodeSpans as zr, sourceItemForCoreRequirement as zt };