...
const protocolBoundary = (seq: number): DerivedEnvelope => ({
  ...
  data: { source: { kind: 'context-guard', plugin: 'context-guard', form: 'notice' }, content: [{ type: 'text', text: PROTOCOL_V3_NOTICE }] },
})
...
function v042ProposalFlow(text = HISTORICAL_TEXT) {
  ...
  events.push({ seq: 3, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'proposal' }, role: 'tool', toolCallId: 'proposal', isError: false, content: [{ type: 'text', text: JSON.stringify(recorded) }] } } })
...
describe('historical v0.4.2 rebind replay compatibility (frozen validator)', () => {
  ...
  it('T03 replays withdrawal and never accepts a model confirmation flag', () => {
    ...
    events.push({ seq: 4, type: 'tool/call', data: { callId: 'withdraw', name: 'context_guard_rebind', arguments: JSON.stringify(args) } },
      { seq: 5, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'withdraw' }, role: 'tool', toolCallId: 'withdraw', isError: false, content: [{ type: 'text', text: JSON.stringify(recorded) }] } } },
...
describe('v0.4.2 tamper resistance under the 0.5 validator', () => {
  it('a changed next_step in a 0.5-era result still replays; changed semantic fields never do', async () => {
    ...
    const evolvedEvents = [...events,
      ...
      { seq: 4, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'c' }, role: 'tool', toolCallId: 'c', isError: false, content: [{ type: 'text', text: JSON.stringify(evolved) }] } } } as DerivedEnvelope,
    ...
    const tamperedEvents = [...events,
      { seq: 3, type: 'tool/call', data: { callId: 'c', name: tool.name, arguments: JSON.stringify(args) } },
      { seq: 4, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'c' }, role: 'tool', toolCallId: 'c', isError: false, content: [{ type: 'text', text: JSON.stringify(tampered) }] } } } as DerivedEnvelope,
      user(5, `确认重绑定 ${response.proposal.id}`)]
