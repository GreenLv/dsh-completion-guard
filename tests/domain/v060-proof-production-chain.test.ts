  ...
  it('a proof-bound certificate is what the Goal gate consumes, and a tampered one is not', async () => {
    ...
    try {
      ...
      const tampered = deriveProjection([
        ...
        { seq: 901, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'goal-cert' }, role: 'tool', toolCallId: 'goal-cert', isError: false, content: [{ type: 'text', text: JSON.stringify(good) }] } } },
