...
function withClosure(extra: DerivedEnvelope[] = []) {
  ...
  const suffix: DerivedEnvelope[] = [
    ...
    { seq: 101, type: 'tool/result', data: { message: { source: { kind: 'tool', callId: 'closure-cp' }, role: 'tool', toolCallId: 'closure-cp', isError: false, content: [{ type: 'text', text: JSON.stringify(record) }] } } },
    ...extra,
  ]
