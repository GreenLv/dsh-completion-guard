...
describe('managed core lock migration and live graph revalidation', () => {
  ...
  it('checks composed policy and actual source roots, not just copied package rows', () => {
    ...
    const wrongRoot = managed.replace(JSON.stringify(active.runtimeRoot), JSON.stringify(join(active.runtimeRoot, 'other')))
    expect(wrongRoot).not.toBe(managed)
    expect(() => verifyComposedHostLockDump(wrongRoot, active.evaluation, active)).toThrow()
  })
})
