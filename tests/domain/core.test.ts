...
describe('domain core', () => {
  ...
  it('replays always-active durable clean bash evidence into a scope certificate', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'Done in 1.2s' }] } } },
    ]
  ...
  it('never promotes a persisted incomplete checkpoint to a certificate', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'ok' }] }, meta: { path: 'src/app.ts' } } },
      ...
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: JSON.stringify({ status: 'incomplete', contract_revision: 1, open_items: ['A001'], rejected_bindings: [] }) }] } } },
    ]
...
因此改用环境内可用的 PowerShell 创建了该文件（\`.NET WriteAllText\` 写入，UTF-8 无 BOM、无末尾换行），确保文件内容严格为 \`guard-test\` 以满足验收标准。`
  ...
  it('does not let pwsh create + read close a bash-required artifact (P0-3 negative)', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'created' }] } } },
      { seq: 4, type: 'tool/call', data: { turn: 1, step: 2, callId: 'c2', name: 'read', arguments: '{"file_path":"C:\\work\\guard-demo.txt"}' } },
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'guard-test' }] }, meta: { path: 'C:\\work\\guard-demo.txt' } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: 'C:\\work' }, true)
  ...
  it('certifies a bash-required artifact with bash + read evidence (P0-3 positive)', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: '[exit code: 0]' }] } } },
      { seq: 4, type: 'tool/call', data: { turn: 1, step: 2, callId: 'c2', name: 'read', arguments: '{"file_path":"C:\\work\\guard-demo.txt"}' } },
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'guard-test' }] }, meta: { path: 'C:\\work\\guard-demo.txt' } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: 'C:\\work' }, true)
  ...
  it('rejects an unrelated bash call as method evidence for an artifact (P0-1 negative)', () => {
    const events = [
      ...
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'created' }] } } },
      { seq: 6, type: 'tool/call', data: { turn: 1, step: 3, callId: 'c3', name: 'read', arguments: '{"file_path":"C:\\work\\guard-demo.txt"}' } },
      { seq: 7, type: 'tool/result', data: { turn: 1, step: 3, message: { source: { kind: 'tool', callId: 'c3' }, role: 'tool', toolCallId: 'c3', isError: false, content: [{ type: 'text', text: 'guard-test' }] }, meta: { path: 'C:\\work\\guard-demo.txt' } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: 'C:\\work' }, true)
    const item = [...derived.projection.items.values()].find((i) => i.kind === 'requirement')
    expect(item?.verification.method).toBe('bash')
  ...
  it('does not treat quoted pwsh strings as commands (P0-1)', () => {
    ...
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'Set-Content C:\\work\\guard-demo.txt now' }] } } },
      { seq: 4, type: 'tool/call', data: { turn: 1, step: 2, callId: 'c2', name: 'read', arguments: '{"file_path":"C:\\work\\guard-demo.txt"}' } },
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'guard-test' }] }, meta: { path: 'C:\\work\\guard-demo.txt' } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: 'C:\\work' }, true)
  ...
  it('certifies a node-run artifact with run + read evidence (P1-1)', () => {
    const events = [
      ...
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'ok' }] }, meta: { path: '/work/script.js' } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: '/work' }, true)
    const item = [...derived.projection.items.values()].find((i) => i.kind === 'requirement')
  ...
  it('v0.2: an install-and-restart task certifies with a shell run and a clean unittest', () => {
    ...
    const make = (command: string, callId: string) => ([
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId }, role: 'tool', toolCallId: callId, isError: false, content: [{ type: 'text', text: 'ok' }] } } },
    ])
  ...
  it('v0.2: a macOS-style pull/apply task certifies with run evidence alone', () => {
    ...
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'Updating..' }] } } },
      ...
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'done' }] } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd }, true)
    const item = [...derived.projection.items.values()].find((i) => i.kind === 'requirement')
  ...
  it('v0.2: a Windows-style pwsh push task certifies with method identity', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'committed' }] } } },
      ...
      { seq: 5, type: 'tool/result', data: { turn: 1, step: 2, message: { source: { kind: 'tool', callId: 'c2' }, role: 'tool', toolCallId: 'c2', isError: false, content: [{ type: 'text', text: 'pushed' }] } } },
    ]
    const derived = deriveProjection(events, OPT_IN, { cwd: 'C:\\work' }, true)
    const item = [...derived.projection.items.values()].find((i) => i.kind === 'requirement')
  ...
  it('v0.2: an install task closes with a dsh CLI run or a clean state check', () => {
    ...
    const make = (command: string, callId: string) => ([
      ...
      { seq: 2, type: 'tool/call', data: { turn: 1, step: 1, callId, name: 'bash', arguments: JSON.stringify({ command }) } },
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId }, role: 'tool', toolCallId: callId, isError: false, content: [{ type: 'text', text: 'done' }] } } },
    ])
  ...
  it('v0.2: rejected bindings and recovery packets carry actionable closing hints', () => {
    const events = [
      ...
      { seq: 3, type: 'tool/result', data: { turn: 1, step: 1, message: { source: { kind: 'tool', callId: 'c1' }, role: 'tool', toolCallId: 'c1', isError: false, content: [{ type: 'text', text: 'ok' }] }, meta: { path: '/work/other.txt' } } },
    ]
