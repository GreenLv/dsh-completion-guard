...
- **Process-verb contract mapping (F3).** Task-level actions — 拉取/获取/同步/更新/下载/安装/部署/上传/提交/推送/发布/升级/重启/重新启动/重载 and `pull`/`fetch`/`clone`/`sync`/`update`/`install`/`deploy`/`commit`/`push`/`release`/`download`/`upload`/`restart`/`reload`/`reboot` — now map captured clauses to the `run` operation, so a successful run in scope closes them (previously an operation-less clause defaulted to the state-verification facet and could not be closed by the work evidence itself). `python -m unittest|doctest|pytest` now counts as a deterministic check.
- **Actionable certification hints.** Rejected checkpoint bindings now carry a `hint` naming the missing facet and a realistic evidence shape (e.g. a whitelisted executable without pipes/`;`), and recovery packets append best-effort `closing hint` lines per open item. This makes the fail-closed reason discoverable without reverse-engineering the guard, and never weakens the contract.
- **Structured meta terminals (defensive).** When a tool/result `meta` carries `exitCode`/`exit_code`/`signal`, those structured facts take precedence over text-marker scanning; text scanning remains the fallback, so a future harness that surfaces run status structurally is trusted directly.
- **Real-workflow fail-closed anchors.** Command forms taken verbatim from the 2026-08-27 macOS sessions (compound `&&`/`;`/pipe inspection, `for` loops, piped config dumps) are locked in as regression anchors: the surface growth must never open them.
- **Manifest-driven command surface.** Every enumeration (file/read tools, run executables, PowerShell externals, clause verb mapping) now lives in `src/domain/manifest.ts` as one data source with a self-validation suite (non-empty, duplicate-free, POSIX/PowerShell parity, verb priority order, patterns compile). Adding a tool or verb is a data change; the manifest ships with the package and is intentionally not runtime-writable, because widening the surface lowers the evidence bar.
- **Informational reports are not contract items.** Pasted acceptance receipts, summaries and logs (markdown headings, bold key/value lines, list/table rows, evidence terms) no longer become per-sentence requirements; any question mark or imperative opening keeps the message a task, so real instructions are never dropped.
- **Help flags are inspection.** `--help`/`-h` no longer count as deterministic checks (a `python -m unittest --help` result previously qualified as verification — a false-path fix).
- **Recovery packet folding.** Oversized open-item/evidence lists fold to the top 8 items / 20 evidence rows with a `checkpoint tool` pointer; closing hints apply only to listed items, never resurrecting folded ones.
- **Checkpoint evidence metadata.** `available_evidence` now includes `outcome` and `capabilities`, so an agent can pick certifying evidence without guessing.
