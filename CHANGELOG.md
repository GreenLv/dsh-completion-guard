...
- **The active cohort is the 33-row DSH 0.1.5-rc.1 core graph, and it says how it was established.** Rows are the exact published npm `dist.integrity` identities. No native macOS or Windows host has loaded that graph in this round, so the cohort records `auditProvenance: registry-derived-pending-native-audit` with an empty `auditedPlatforms` list while still accepting evaluation on both platforms. The provenance value is bound into `hostLockDigest`, so a certificate from a registry-derived graph can never be presented as a native pass. The five older cohorts stay recorded as historical identities and fail closed. `dshmarket` is no longer a cohort row: market identity is verified independently by the action adapter.
