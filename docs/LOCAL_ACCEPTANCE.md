...
This entry separates exact candidate source, CI, package, native-host and model evidence. It does not itself establish a tag, npm publication or daily installation. The shared core/v2 source and mirror bytes are pinned to Codex Context Guard commit `94ebdb00a74ad0707f3c62dec0cb109cf6d2f4dd` by `tests/fixtures/conformance/core_v2/UPSTREAM_PIN.json`; the historical v1/digest pin is unchanged. Matching shared files does not establish complete runtime or feature equivalence.

Committed candidate `583035bd90b8ee589d01b12487a057b655d43e41` passed its complete local deterministic source matrix and all ten jobs in [candidate CI](https://github.com/GreenLv/dsh-completion-guard/actions/runs/35520634411). Its canonical clean-source npm tgz (SHA-256 `732e6a904ad0fabd1f58281017404ec23416fe84d400b6cc870950bb10f649f9`) passed the macOS `host_bound_v070` native profile: 34 required gates and cleanup passed on the exact artifact. That annex skipped `real_model_request`. That historical artifact has no Windows exact-artifact native or complete fixed-model acceptance. npm publication, tag, GitHub Release and daily-profile installation require separate final-identity evidence. Later source or packaged-document changes create a new candidate: the 583035b artifact and macOS annex remain historical evidence for their original bytes, not acceptance of the changed package.

A later source candidate, `519c9daffd011fdd297d12173dd62f9f8e67d2a3`, passed its complete local deterministic matrix and [candidate CI](https://github.com/GreenLv/dsh-completion-guard/actions/runs/35561059326). Its frozen CI tgz (SHA-256 `9f281f177d3b379ba1349f04206bc08d7d4949c50b73aba3ecdf1e3213f17d9d`) passed all 34 required macOS host-bound gates and cleanup; that native annex also skipped `real_model_request`. Separate macOS model captures established supported observer-backed edit/test/readback and future-observation/resume controls, but did not complete the required integrated A12 workflow or four-platform acceptance. One ordinary-edit capture lacked the required file observation; a persistent nonzero-test capture timed out before its control sequence. These remain recorded gaps, not passing results. Renderer-only nonzero output cannot certify that a named package test script started. New pin or packaged-document bytes require a new artifact identity; the 519c9da results retain their original subjects.

In 0.7.0 ordinary edits, tests and Git operations run through DSH Host tools. The adapter reads the root source, confirmed Session events, persisted Host call/results and independent readback. A relative file or directory keeps its root-time Session locator and canonical filesystem containment; an opaque test or measurement target cannot be relabeled as a file. A successful mutation of a forbidden file stays a sourced completion violation even when an allowed edit succeeded. Old ordinary `context_guard_action` and `context_guard_evidence` calls create no business effect and return migration guidance; read-only file, Git and package-script readiness observations do not perform ordinary mutations.

Stop and checkpoint distinguish a ready current action from future observation, unmet time or approval, and insufficient evidence. A later authorization does not backfill an earlier Stop. Root-scoped persistence, pause, cancellation and resume affect only the requirements in scope when recorded. An explicit observer method remains its own sourced obligation. A named foreground `npm test` or `pnpm test` can support a run requirement only with audited Host renderer bytes and a trusted terminal result; a run-and-report requirement differs from a pass target. Creating a file still needs trustworthy pre-effect absence. Arbitrary package-script metrics, nonzero exit codes inferred only from stdout markers, and unobserved Host effects remain insufficient. Historical v5 and pre-v6 generic records are preserved but cannot be upgraded into current authority.

The versioned native entrypoint is `scripts/native_acceptance.py --gate-profile host_bound_v070`. Run `--preflight` with the intended source, artifact, host cohorts and unused result path before the actual native run. Its `dsh-host-bound/v4` annex binds the source commit, artifact digest, driver files, host locks, platform and cleanup. The nine Web and Headless cases cover real host registration, ordinary file/test readback, Stop, migration, Goal, locally mocked adopted release protection and history; Web restart adds a tenth persisted case. The mock release case performs no public registry request. A source replay or portable/CI pass is not a native, model or release pass.
