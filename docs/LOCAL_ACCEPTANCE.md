...
This is a local source-candidate record, not a release or native-platform acceptance. The shared core/v2 source and mirror bytes are bound to Codex Context Guard commit `f0196b690c091efd8bca8e17e793c4b3e74dabc3` by `tests/fixtures/conformance/core_v2/UPSTREAM_PIN.json`; the historical v1/digest `UPSTREAM_PIN.json` keeps its prior identity. Every mirrored fixture and runtime JSON copy is checked against the committed upstream bytes. This is source-mirror identity, not a claim of complete runtime equivalence, installation, native acceptance, or publication. DSH product commit, clean-source artifact and CI results are separate follow-up evidence kept in the coordinated handoff.
