...
Local replay entry: build with `tsdown`, then run `node scripts/replay_raw_v2.mjs INPUT.json`, where the input contains `root`, `final`, and optional synthetic host `events`. It invokes the actual Stop callback before appending `turn/end`, then reports the Stop-time core snapshot/projection separately from the post-turn trusted-delivery projection. The result also includes the Stop/Goal decision. This is synthetic source evidence, not a real model, installed plugin, or native host run. Earlier candidate bytes passed the complete local deterministic source matrix, including tests that commit or push only inside isolated disposable Git fixtures, plus the shared core fixtures, typecheck, lint, and a byte-stable double build. An isolated package inventory check ran without publication or daily installation. A precommit check confirmed that the canonical release packer refuses a dirty product worktree; any subsequent clean-source artifact result is recorded separately outside this repository. Exact results for the current repaired bytes, file hashes, independent synthetic host replays, and remaining evidence gaps are recorded in the coordinated handoff outside this repository.

The 0.7 native profile uses `scripts/native_acceptance.py --gate-profile host_bound_v070` with exact source/artifact/cohort inputs and a fresh external result path. Its `dsh-host-bound/v4` annex is checked by the repository-owned `scripts/validate_native_v070.py` against a clean source commit, the exact tgz, the intended platform and both expected host-lock digests. The v4 driver digest binds the main probe, its host helper, and the isolated mock-release helper by filename and SHA-256. The nine Web and Headless cases cover real host tool registration, ordinary file/test readback, Stop, migration, Goal, a locally mocked adopted release contract, and history; Web restart adds a tenth persisted case. The mock release case runs no public publish command or registry request. This no-model probe is distinct from real-model acceptance. The older `host_bound` profile and v3 consumer retain their historical contract; a v3 annex cannot pass the v4 validator. A new runtime build or packaged byte requires a new exact artifact and fresh native evidence.

The current v6 source-capture regression distinguishes an imperative scoped by a fronted workspace location from a future preface or a report about someone else's action. It preserves independently sourced edit, test, file-readback, and final-answer clauses; a dependent host-tool method or completion phrase cannot replace those outcomes. A successful later read does not prove that a created file was absent before its write. The package-script readiness observer only supports its audited test/benchmark scripts: another explicitly named script remains a current request but returns `readiness_script_capability_unavailable` rather than borrowing an unrelated script's success. The synthetic capture tests exercise these positive and negative source roles; they do not certify arbitrary numeric output or constitute a new real-model run.

An affected source repair keeps an explicitly requested read-only observer method as a separate root-sourced predicate. It requires a later, matched and persisted Host observer result for the same test or file duty; pre-v6 generic text cannot be replayed into that predicate. A named ordinary foreground `npm test` or `pnpm test` can use a verified active Host renderer without requiring a Guard readiness observer the root did not request. The byte attestation covers the installed bash or PowerShell renderer and shared shell module, not package version strings alone. For those exact bytes, a complete markerless result proves exit zero; a nonzero marker is text that stdout can imitate, so the current Host result does not certify its numerical exit or a truthful failure report. A settled call remains observed even when the specific test outcome is insufficient, and it does not authorize repair or an automatic rerun. The minimum Host improvement is a persisted same-call structured start, completion, termination and exit code with stdout and stderr separated. Synthetic Windows-shaped source tests do not replace native Windows acceptance; an rc.1 byte match does not claim that rc.1 was the active local runtime.

The current source repair assigns a distinct delivery fact to each information requirement satisfied by one trusted final answer, while retaining the same final-answer source. Focused source tests cover two requirements delivered together, the pre-delivery watermark, a later root that cannot reuse an earlier answer, replay, and rejection of a genuinely duplicated fact identity. A read-only replay of an earlier private model Session archive no longer fails on duplicate delivery identity; its separate test predicate remains insufficient because a later unknown test observation supersedes an earlier success. This replay checks repaired source bytes, not the previously frozen `c4266b9` package. Its native and model receipts remain evidence only for that older artifact; the repaired runtime still needs a new commit, clean-source package, CI, and native acceptance before those claims can be made for it.

The subsequent root-control source candidate keeps persistence, pause, cancel and resume tied to immutable root sequence, work unit, requirement IDs and receipt-time scope. Original-root coordination can bind a directly related same-object test to its edit; a later control cannot invent that relationship. A complete sourced compound control does not leave duplicate generic business requirements in the core projection, while a separate publish request and quoted control text retain their own reading. Focused Session tests check completed-test certification, historical supersession, multiple required children, an unrelated sibling, explicit proof, deleted-item unknown coverage and a forged different-target successor. The shared ten-file source mirror is now bound to the committed upstream bytes by the core/v2 pin; these source tests do not update the older packaged artifact or establish new native/model acceptance.

The shared cancellation projection removes only its sourced ordinary controlled requirements from current continuation. DSH preserves the native Session items and work-unit ledger as history; it does not infer Goal completion or release-contract revocation from that projected cancellation. The Codex adapter may end its active work unit after a whole-task cancellation, which is a host lifecycle choice rather than a shared core/v2 assertion.
