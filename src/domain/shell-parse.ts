...
export function parseShellCommand(command: string): ParsedShell {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token.kind === 'word' && /^\d+$/.test(token.value) && tokens[index + 1]?.kind === 'op' &&
      ...
      return unsupported('file-descriptor-prefixed redirect is not in the v0.1 subset')
    }
    if (token.kind === 'op') {
      if (token.value === '>') {
  ...
  if (!SHELL_FILE_TOOLS.has(exe) && !SHELL_RUN_EXECUTABLES.has(exe)) {
...
/**
 * Parse one PowerShell command against the v0.1 subset: a single, directly
 ...
 * parameter, all literal. Multi-statements (`;`), pipelines (`|`), the call
 * operator (`&`), script blocks, dot sourcing, .NET/dynamic invocation,
...
export function parsePwshCommand(command: string): ParsedShell {
  ...
  const { words, malformed } = tokenizePwsh(command)
  ...
  if (!spec) return unsupported(`cmdlet '${cmdletRaw}' is not in the v0.1 whitelist`)
  if (words[0].quoted) return unsupported('quoted cmdlet is not in the v0.1 subset')
